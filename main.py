...
pygame.display.set_caption('Voice Controlled Games')
# background colors
COLORS = {
...
def start_game(game_name):
    ...
    if not game_name:
        speak("Please say a game name.")
        return

    # find the closest matching game
    matched_game = find_closest_game(game_name)

    if matched_game:
        print(f"Starting game: {matched_game}")

        if matched_game == "word detective":
            current_game = "word detective"
            word_detective()
        elif matched_game == "spellbee":
            current_game = "spellbee"
            spellbee()
        elif matched_game == "flash cards":
            current_game = "flash cards"
            flash_cards()
    else:
        speak("I didn't recognize that game. Please try again.")
        show_menu()
...
def get_voice_input():
    ...
    with sr.Microphone() as source:
        ...
        render_text("Listening...", (WINDOW_WIDTH // 2, WINDOW_HEIGHT // 2),
                    COLORS['primary'])  # neeche wala listening
        pygame.display.flip()
    ...
    while game_running:
        ...
        while not round_complete and attempts < 3:  # Maximum 3 attempts per round(edit kr lena)
            ...
            for i, hint in enumerate(hints[:hints_used + 1]):
                ...
                WINDOW_WIDTH // 2 - hint_text.get_width() // 2, 250 + i * 60))  # Increased spacing between hints
...
def voice_control():
    user_input = input("Enter the word as spoken: ")
    return user_input
...
def spellbee():
    ...
    # levels hta diya
    words = [
        "bicycle", "butterfly", "mountain", "chocolate",
        "conscious", "rhythm", "symphony", "phenomenon", "anonymous",
        "cat", "chromosome", "democracy", "hierarchy", "hygiene",
        "tree", "millennium", "occurrence", "parliament", "prejudice"
    ]
...
games = ["word detective", "spellbee", "flash cards"]
